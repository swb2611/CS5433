...
def send_message(dest, type, message):
    ...
    """
    try:
        print(requests.post(dest + "p2pmessage/" + type + "/" +
                            str(config.receiving_port), data=str(message), timeout=2).text)
...
def gossip_message(type, message):
    ...
    for each_peer in config.PEERS.keys():
        if config.node_id != each_peer:
            send_message(config.PEERS[each_peer], type, message)
