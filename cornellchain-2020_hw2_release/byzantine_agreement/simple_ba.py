...
class SimplePKIBA:
    ...
    def __init__(self, sender):
        ...
        # maps proposed messages to list of accepted (node_id, signature) tuples, corresponding to valid votes
        self.signatures = {}
        self.sender = sender
        ...
        if config.node_id == sender:
            # for demo purposes, we'll vote for a random string
            string_to_vote_for = str(int(random.random() * 2 ** 256))
            self.votes[string_to_vote_for] = set([config.node_id])
            proposal_sig = util.sign_message(
                string_to_vote_for, config.SECRET_KEYS[config.node_id])
            self.signatures[string_to_vote_for] = [
                (config.node_id, proposal_sig)]
    ...
    def broadcast_votes_for(self, round, votes):
        ...
        for proposal in votes:
            gossip.gossip_message(
                "ba-vote", json.dumps([proposal, self.signatures[proposal]]))
    ...
    def process_vote(self, vote):
        ...
        for signature in signatures:
            ...
            if util.is_message_signed(proposal, proposal_sig, config.PUBLIC_KEYS[node_id]):
                ...
                print("[byz-ag] Signed proposal message accepted",
                      proposal, signature)
            else:
                print("[byz-ag] Error: Signed proposal message rejected",
                      proposal, signature)
    ...
    def run_protocol_loop(self):
        ...
        while not self.is_done():
            ...
            while not synchrony.should_send():
                time.sleep(.2)
                print("[byz-ag] waiting to send votes in round",
                      self.curr_round_number)
