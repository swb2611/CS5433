...
class Blockchain(persistent.Persistent):
    ...
    def get_chain_ending_with(self, block_hash):
        ...
        target_block = 0
        blockheightlist = self.get_heights_with_blocks()
        block_hash_list = []

        for each_height in range(len(blockheightlist)):
            for each_blocks in self.get_blockhashes_at_height(each_height):
                if block_hash == each_blocks:
                    target_block = self.blocks[each_blocks]
                    break

        if target_block == 0:
            return []

        block_hash_list.append(target_block.hash)

        while not target_block.is_genesis:
            change = False
            for each_block in self.get_blockhashes_at_height(target_block.height - 1):
                if target_block.parent_hash == each_block:
                    target_block = self.blocks[each_block]
                    block_hash_list.append(target_block.hash)
                    change = True
            if change == False:
                break
